...
[role="lead"]
Protégez toutes les applications en effectuant des copies Snapshot et des sauvegardes à l'aide d'une stratégie de protection automatisée ou ad hoc. Vous pouvez utiliser l'interface utilisateur du centre de contrôle Astra ou https://["API de contrôle Astra"^] pour protéger les applications.
...
IMPORTANT: Si votre application utilise une classe de stockage soutenue par `ontap-nas-economy` conducteur, les politiques de protection ne peuvent pas être utilisées. Utilisez une autre classe de stockage si vous souhaitez une règle de protection.
...
IMPORTANT: Si votre application utilise une classe de stockage soutenue par `ontap-nas-economy` pilote, les instantanés ne peuvent pas être utilisés. Utilisez une autre classe de stockage pour les snapshots.
...
IMPORTANT: Si votre application utilise une classe de stockage soutenue par `ontap-nas-economy` pilote, les sauvegardes sont perturbatrices et l'application sera indisponible jusqu'à la fin de l'opération de sauvegarde.
