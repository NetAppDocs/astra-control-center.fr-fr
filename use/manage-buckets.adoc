...
NOTE: Lorsque vous ajoutez un compartiment, sélectionnez le fournisseur de compartiment approprié et fournissez les informations d'identification correctes pour ce fournisseur. Par exemple, l'interface utilisateur accepte NetApp ONTAP S3 comme type et accepte les informations d'identification StorageGRID ; cependant, cela entraînera l'échec de toutes les futures sauvegardes et restaurations d'applications utilisant ce compartiment. Voir la link:../release-notes/known-issues.html["Notes de version"].
