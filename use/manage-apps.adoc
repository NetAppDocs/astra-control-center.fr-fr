...
[role="lead"]
Après vous link:../get-started/add-cluster.html["Ajoutez un cluster à la gestion Astra Control"], Vous pouvez installer des applications sur le cluster (en dehors d'Astra Control), puis aller à la page applications d'Astra Control pour définir les applications et leurs ressources.
...
Après vous l'avez link:../get-started/add-cluster.html["a ajouté votre cluster"] Avec Astra Control, vous pouvez installer des applications ou gérer des applications existantes sur le cluster. Toute application dont la portée est étendue à un ou plusieurs espaces de noms peut être gérée.
...
* (Facultatif) Une étiquette Kubernetes sur n'importe quel link:../use/manage-apps.html#application-management-requirements["Ressources Kubernetes prises en charge"].
+
...
* link:../use/manage-apps.html#tech-preview-define-an-application-using-a-kubernetes-custom-resource["(Aperçu technique) définissez une application à l'aide d'une ressource personnalisée Kubernetes"]
