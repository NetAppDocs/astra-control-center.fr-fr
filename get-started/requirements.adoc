...
+
** Le jeton du fichier de configuration par défaut VMware TKG et TKGi expire dix heures après le déploiement. Si vous utilisez des produits du portefeuille Tanzu, vous devez générer un fichier de configuration Tanzu Kubernetes Cluster avec un jeton sans expiration afin d’éviter les problèmes de connexion entre Astra Control Center et les clusters d’applications gérés. Pour obtenir des instructions, visitez  https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-2.html["la documentation produit VMware NSX-T Data Center"^].
