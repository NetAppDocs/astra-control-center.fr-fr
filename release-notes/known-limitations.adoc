...
Si vous souhaitez gérer un cluster sur une autre instance d'Astra Control Center, vous devez d'abord link:../use/unmanage.html#unmanage-a-cluster["annuler la gestion du cluster"] depuis l'instance sur laquelle il est géré avant de le gérer sur une autre instance. Après avoir retiré le cluster de la gestion, vérifiez qu'il n'est plus géré en exécutant cette commande :
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["Installez la dernière version d'Astra Trident"^].
...
Assurez-vous de link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["entrez les valeurs correctes"] le faire lors de l’établissement d’une connexion.
